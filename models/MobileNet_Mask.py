...
class MobileNet_Mask(nn.Module):
    def __init__(self,
                 num_classes=1000,
                 width_mult=1.0,
        ...
        """
        super(MobileNet_Mask, self).__init__()
        ...
        #self.input_channel = self.input_channel if 0 not in self.mask_layer else self.input_channel + 1 
    ...
    def _make_layer(self, index):
        ...
        self.input_channel = self.input_channel if index-1 not in self.mask_layer else self.input_channel + 1 
        Blocks = []
        for i in range(n):
...
if __name__ == '__main__':
    model = MobileNet_Mask(img_channel=4, mask_layer=[0,1])
    #print(model)
