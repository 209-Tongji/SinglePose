...
from transform import SimpleTransformMask

import cv2

class MaskCOCO2(data.Dataset):
    CLASSES = ['person']
    ...
    def __init__(self,
        ...
        self.transformation = SimpleTransformMask(
    ...
    def __getitem__(self, idx):
        ...
        '''
        Add person mask to the origin image
        '''
        mask = self._coco.annToMask(label)
        mask = mask * 255
        # origin_img = cv2.imread(img_path)
        # new_image = origin_img * (mask[:,:,None].astype(origin_img.dtype))
        # cv2.imwrite("mask{:0>5d}.jpg".format(idx), new_image)
        img = cv2.cvtColor(cv2.imread(img_path), cv2.COLOR_BGR2RGB)
        
        target = self.transformation(img, label, mask)
...
if __name__ == '__main__':
    ...
    train_dataset.__getitem__(1280)